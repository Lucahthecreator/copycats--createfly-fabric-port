...
public class CopycatVerticalStairsModelPart implements CopycatModelPart {
    ...
    public void emitCopycatQuads(BlockState state, CopycatRenderContext context, BlockState material) {
        ...
        switch (shape) {
            case STRAIGHT -> {
                ...
                CopycatStairsModelPart.assembleStraight(context, transform);
            }
            case INNER_BOTTOM, INNER_TOP -> {
                ...
                CopycatStairsModelPart.assembleInnerLeft(context, transform);
            }
            case OUTER_BOTTOM, OUTER_TOP -> {
                boolean flipY = shape.isTop();
                boolean flipX = side.isRight();
                GlobalTransform transform = t -> t.rotateX(90).rotateZ(90).flipX(flipX).flipY(flipY).rotateY(facing);
                CopycatStairsModelPart.assembleOuterLeft(context, transform);
            }
        }
    }
}
