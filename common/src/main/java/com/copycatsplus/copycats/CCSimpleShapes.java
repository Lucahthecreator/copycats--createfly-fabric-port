...
public class CCSimpleShapes {
    ...
    public static final Map<Direction, Map<Direction, MutableShape>> HALF_PANEL =
            forAll(CopycatHalfPanelBlock.FACING, direction -> forAll(CopycatHalfPanelBlock.OFFSET,
                    offset -> {
    ...
    public static final Map<Direction, Map<Half, MutableShape>> SLOPE =
            forHorizontalDirections(forHalves(shape(
                    IntStream.range(0, SLOPE_SUBDIVISIONS)
                            .mapToObj(i -> aabb(16, i + 1, 16.0 / SLOPE_SUBDIVISIONS).move(0, 0, i * 16.0 / SLOPE_SUBDIVISIONS)).toArray(MutableAABB[]::new)
            ).outline(
    ...
    public static class MutableShape implements AssemblyTransform.Transformable<MutableShape> {
        ...
        public VoxelShape toShape() {
            if (output == null) {
                ...
                if (!outlines.isEmpty()) {
                    shape = new NoneVoxelShape(shape, outlines.stream().map(p -> Pair.of(p.getFirst().toVec3(), p.getSecond().toVec3())).collect(Collectors.toList()));
                }
                output = shape;
            }
            return output;
