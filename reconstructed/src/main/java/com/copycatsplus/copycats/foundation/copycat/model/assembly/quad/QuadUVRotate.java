    ...
    public boolean transformQuad(MutableQuad quad, TextureAtlasSprite sprite) {
        if (quad.computeLightFace() == this.face) {
            for (int vertex = 0; vertex < 4; ++vertex) {
                ...
                quad.vertices.get((int)vertex).uv.u = sprite.getU((float)((double)u * cos - (double)v * sin + (double)this.pivotU));
                quad.vertices.get((int)vertex).uv.v = sprite.getV((float)((double)u * sin + (double)v * cos + (double)this.pivotV));
            }
        }
        return true;
    }
