    ...
    public boolean transformQuad(MutableQuad quad, TextureAtlasSprite sprite) {
        if (quad.computeLightFace() == this.face) {
            for (int vertex = 0; vertex < 4; ++vertex) {
                float u = SpriteShiftEntry.getUnInterpolatedU((TextureAtlasSprite)sprite, (float)quad.vertices.get((int)vertex).uv.u) + this.offsetU;
                float v = SpriteShiftEntry.getUnInterpolatedV((TextureAtlasSprite)sprite, (float)quad.vertices.get((int)vertex).uv.v) + this.offsetV;
                quad.vertices.get((int)vertex).uv.u = sprite.getU(u);
                quad.vertices.get((int)vertex).uv.v = sprite.getV(v);
            }
        }
        return true;
    }
